class Messages::MessageBuilder
  ...
  def perform
    ...
    # Render template content for UI preview if applicable
    if @conversation.inbox.whatsapp? && @whatsapp_resolver
      @message.content = @whatsapp_resolver.rendered_content
    end
  ...
  def template_params
    ...
    template_params_hash = JSON.parse(@params[:template_params].to_json).with_indifferent_access
    
    # Auto-fill variables for WhatsApp templates if they are missing or placeholders are used
    if @conversation.inbox.whatsapp? && template_params_hash[:name].present?
      template_params_hash[:processed_params] = resolve_whatsapp_variables(template_params_hash)
    end

    { additional_attributes: { template_params: template_params_hash } }
  end

  def resolve_whatsapp_variables(template_params_hash)
    channel = @conversation.inbox.channel
    template = channel.message_templates&.find do |t|
      t['name'] == template_params_hash[:name] && t['language'] == template_params_hash[:language]
    end

    return template_params_hash[:processed_params] if template.blank?

    @whatsapp_resolver = Whatsapp::TemplateVariableResolverService.new(
      template: template,
      contact: @conversation.contact,
      params: template_params_hash[:processed_params] || {}
    )
    @whatsapp_resolver.resolve
  end
