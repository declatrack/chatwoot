module AttachmentConcern
  ...
  def validate_and_prepare_attachments(actions, record = nil)
    ...
    sanitized = actions.map do |action|
      action_hash = action.respond_to?(:to_unsafe_h) ? action.to_unsafe_h.with_indifferent_access : action
      next action_hash unless action_hash[:action_name] == 'send_attachment'

      result = process_attachment_action(action_hash, record, blobs)
