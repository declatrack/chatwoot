class Api::V1::Accounts::ContactsController < Api::V1::Accounts::BaseController
  ...
  before_action :ensure_can_edit_contact, only: [:update, :destroy_custom_attributes]
  ...
  def ensure_can_edit_contact
    return if Current.user.blank?
    return unless Current.user.is_a?(User)

    account_user = Current.user.account_users.find_by(account_id: Current.account.id)
    return if account_user.blank? || account_user.administrator?
    return if account_user.has_permission?('can_edit_contacts')

    render json: { error: 'Você não tem permissão para editar contatos.' }, status: :forbidden
  end
end
