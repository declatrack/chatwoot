class Api::V1::Accounts::InboxesController < Api::V1::Accounts::BaseController
  ...
  def create_whatsapp_template
    return render status: :unprocessable_entity, json: { error: 'Template creation is only available for WhatsApp channels' } unless @inbox.channel.is_a?(Channel::Whatsapp)

    @inbox.channel.create_template(params.permit!)
    render status: :ok, json: { message: 'Template creation initiated successfully' }
  rescue StandardError => e
    render status: :internal_server_error, json: { error: e.message }
  end
  ...
  def inbox_attributes
    [:name, :avatar, :greeting_enabled, :greeting_message, :enable_email_collect, :csat_survey_enabled,
     ...
     :lock_to_single_conversation, :portal_id, :sender_name_type, :business_name, :enable_auto_signature,
