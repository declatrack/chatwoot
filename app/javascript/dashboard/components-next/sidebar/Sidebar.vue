...
const menuItems = computed(() => {
  return [
    ...
    if (item.name === 'Inbox' && !canViewInboxTab.value) return false;
    if (item.name === 'Contacts' && !canViewContacts.value) return false;
    if (item.name === 'Captain' && !canViewCaptain.value) return false;
    if (item.name === 'Settings' && !canViewSettings.value) return false;
    return true;
  }).map(item => {
    if (item.name === 'Conversation') {
      item.children = item.children.filter(child => {
        if (child.name === 'Mentions' && !canViewMentionsTab.value) return false;
        if (child.name === 'Unattended' && !canViewUnattendedTab.value) return false;
        if (child.name === 'Teams' && !canViewTeamsTab.value) return false;
        if (child.name === 'Channels' && !canViewChannelsTab.value) return false;
        return true;
      });
    }
    return item;
