class AutomationRuleListener < BaseListener
  ...
  def message_created(event)
    ...
    rules.each do |rule|
      AutomationRuleLog.create_log(rule, account, message.conversation) do |log|
        ...
        if conditions_match.present?
          ::AutomationRules::ActionService.new(rule, account, message.conversation, log: log).perform
          log.success!
        else
          log.ignored!
        end
  ...
  def process_conversation_event(event, event_name)
    ...
    rules.each do |rule|
      AutomationRuleLog.create_log(rule, account, conversation) do |log|
        conditions_match = ::AutomationRules::ConditionsFilterService.new(rule, conversation, { changed_attributes: changed_attributes }).perform
        if conditions_match.present?
          AutomationRules::ActionService.new(rule, account, conversation, log: log).perform
          log.success!
        else
          log.ignored!
        end
