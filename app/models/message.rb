...
class Message < ApplicationRecord
  ...
  has_many :message_delivery_logs, dependent: :destroy_async
  ...
  def create_pending_delivery_log
    # Only log outgoing/template messages that will be routed to an external channel.
    # Incoming messages, activity messages, and private notes are excluded.
    return unless outgoing? || template?
    return if private?

    MessageDeliveryLog.log_attempt(message: self, status: 'pending')
  end
