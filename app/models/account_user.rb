...
class AccountUser < ApplicationRecord
  ...
  TEAM_CONVERSATION_VISIBILITY_OPTIONS = %w[team_only team_and_unassigned all_inbox].freeze

  def has_permission?(key)
    if administrator?
      return false if key.to_s == 'can_view_only_assigned'
      return 'all_inbox' if key.to_s == 'team_conversation_visibility'

      return true
    end

    stored = self[:permissions] || {}
    stored.key?(key.to_s) ? stored[key.to_s] : PERMISSION_DEFAULTS.fetch(key.to_s, true)
  end

  def resolved_permissions
    PERMISSION_DEFAULTS.each_with_object({}) do |(key, default_val), hash|
      stored = self[:permissions] || {}
      hash[key] = stored.key?(key) ? stored[key] : default_val
    end
  end
