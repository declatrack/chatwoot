class Whatsapp::TemplateProcessorService
  ...
  def process_header_components(processed_params, template)
    ...
    if processed_params['header'].present?
      header_params.concat(build_header_params(processed_params['header']) || [])
    end

    # Add text variables if present
    if processed_params['header_text'].present?
      text_params = processed_params['header_text'].filter_map do |key, value|
        next if value.blank?
      ...
      header_params.concat(text_params)
    end
