class Whatsapp::Providers::WhatsappCloudService < Whatsapp::Providers::BaseService
  ...
  def create_template(template_params)
    request_body = {
      ...
      parameter_format: template_params[:parameter_format] || 'named',
    ...
    if response.success?
      SyncWhatsappTemplatesJob.perform_later(whatsapp_channel.id)
    else
      err_msg = error_message(response)
      err_msg ||= response.parsed_response&.dig('error', 'error_user_msg')
      err_msg ||= response.parsed_response.to_s
      err_msg ||= 'Failed to create template on Meta'
      raise err_msg
    end
  ...
  def send_text_message(phone_number, message)
    response = HTTParty.post(
      ...
      body: {
        ...
        to: phone_number,
        text: { body: message_body_with_signature(message) },
        type: 'text'
      }.to_json
    )
  ...
  def send_attachment_message(phone_number, message)
    ...
    type_content['caption'] = message_body_with_signature(message) unless %w[audio sticker].include?(type)
  ...
  def message_body_with_signature(message)
    body = message.outgoing_content
    return body unless whatsapp_channel.inbox&.enable_auto_signature
    return body unless message.sender.is_a?(User)

    agent_name = message.sender.display_name.presence || message.sender.name
    "*#{agent_name}* diz:\n\n#{body}"
  end
end
